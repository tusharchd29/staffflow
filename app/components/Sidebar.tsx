...
const nav = [
  ...
  { label: "Worker Board", icon: CalendarCheck, href: "/consultants" },
...
export default function Sidebar() {
  ...
  return (
    <>
      <button onClick={() => setOpen(!open)} style={{ background: "var(--espresso)", color: "var(--sand)" }}
        className="fixed top-4 left-4 z-50 p-2 rounded-lg md:hidden">
        {open ? <X size={20} /> : <Menu size={20} />}
      </button>

      <aside style={{ background: "var(--espresso)", borderRight: "1px solid var(--espresso-mid)" }}
        className={`fixed top-0 left-0 h-screen w-64 flex flex-col z-40 transition-transform duration-300 ${open ? "translate-x-0" : "-translate-x-full"} md:translate-x-0`}>
        {/* Logo */}
        <div style={{ borderBottom: "1px solid rgba(255,255,255,0.08)" }} className="px-6 py-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div style={{ color: "var(--sand)", fontFamily: "'Playfair Display', serif", fontSize: 18, fontWeight: 600 }}>StaffFlow</div>
              <div style={{ color: "rgba(255,255,255,0.35)", fontSize: 11 }}>Labour Max · Brampton, ON</div>
            </div>
          </div>
        </div>
        ...
        <nav className="flex-1 px-3 py-4 overflow-y-auto">
          <div style={{ color: "rgba(255,255,255,0.25)", fontSize: 10, letterSpacing: "0.1em", textTransform: "uppercase" }} className="px-3 mb-3">Main Menu</div>
        ...
        <div style={{ borderTop: "1px solid rgba(255,255,255,0.08)", padding: "16px 12px" }}>
          <Link href="/settings" onClick={() => setOpen(false)} style={{
            ...
            borderRadius: 8, marginBottom: 4, color: pathname === '/settings' ? "var(--amber-light)" : "rgba(255,255,255,0.4)",
            fontSize: 13, cursor: "pointer", textDecoration: "none",
            background: pathname === '/settings' ? "rgba(196,132,29,0.18)" : "transparent"
          }}>
          ...
          <div style={{ marginTop: 12, padding: "8px 10px", background: "rgba(255,255,255,0.04)", borderRadius: 8, display: "flex", alignItems: "center", gap: 10 }}>
            <div style={{ width: 32, height: 32, borderRadius: "50%", background: "linear-gradient(135deg, var(--amber), var(--clay))", display: "flex", alignItems: "center", justifyContent: "center", color: "white", fontWeight: 700, fontSize: 13, flexShrink: 0 }}>AM</div>
            <div>
